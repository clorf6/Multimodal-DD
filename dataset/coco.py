...
class coco_val(Dataset):
    def __init__(self, transform, dataset_root, max_words=30, prompt=''):
        ...
        self.image_idx = random.sample(range(len(self.data)), 100)
        self.caption_idx = [random.randint(0, 4) for _ in range(100)] 

    def __len__(self):
        return 100

    def __getitem__(self, index):
        image_path = os.path.join(self.image_root, self.data[self.image_idx[index]]['image'])
        image = Image.open(image_path).convert('RGB')
        image = self.transform(image)
        captions = self.data[self.image_idx[index]]['captions']
        for caption in captions:
            caption = self.prompt + pre_caption(caption, self.max_words)
        if len(captions) < self.caption_idx[index] + 1:
            captions = ''
        else:
            captions = captions[self.caption_idx[index]]
        return image, captions
